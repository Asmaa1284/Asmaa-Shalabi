...
public class FoodTester {
	...
	public static void main(String[] args) {
		...
		Thread cooker = new CookThread(foodList);  
		Thread server = new ServeThread(foodList);
		cooker.start();
		server.start();
		...
		int totalCookTime = foodList.stream().mapToInt((f -> f.getCookTime())).sum();
		int totalServeTime = foodList.stream().mapToInt((f -> f.getServeTime())).sum();
