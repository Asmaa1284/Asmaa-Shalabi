...
public class ProductBigDataQuestions {
    ...
    public static int q1() {
        ...
        return productList.stream().reduce(0,
        		(total,product)-> total+ product.getNumberOfChemicals(),
        		(total1,total2)->total1+total2); // placeholder so the code will compile; replace with your own code
    }
    public static int q2() {
        // QUESTION 2: How many different companies are in the dataset?
    	 return (int) productList.stream()
    			 .map(Product::getCompany).distinct()
    			 .count(); // placeholder so the code will compile; replace with your own code
    }
    public static long q3() {
        // QUESTION 3: How many products have 4 or more chemicals?
    	 return productList.stream().filter(product->
    	 product.getNumberOfChemicals()>=4).count(); // placeholder so the code will compile; replace with your own code
    }
    public static Map<String, List<Product>> q4() {
        ...
        // Hint: use Collectors.groupingBy!
    	 return productList.stream().collect(
    			 Collectors.groupingBy(Product::getCompany)
    			 ); // placeholder so the code will compile; replace with your own code
	...
	public static String q5(Map<String, List<Product>> companyProductMap) {
		...
		return  companyProductMap.entrySet().stream().max((w1,w2)->
		
		 Integer.compare(w1.getValue().size(), w2.getValue().size())).get().getKey();// placeholder so the code will compile; replace with your own code
    }
    public static void q6( Map<String, List<Product>> chemicalProductMap) {
    	...
    	productList.stream().forEach(chList->{
    		 chList.getChemicals().stream().collect(Collectors.toSet()).forEach(s->{
    		 if(!chemicalProductMap.containsKey(s)) {
    			 chemicalProductMap.put(s, productList);
    		 }else {
    			 chemicalProductMap.get(s).add(chList);
    		 }
    		 }
    		 );
    ...
    public static String q7() {
    	...
    	// to compare chemical names based on the size of the list of products.
    	 return chemicalProductMap.entrySet().stream().max((w1,w2)->
