...
public static <T>List<T> selectUniqueWinners(RandomDrawingInterface<T> object,int num){
	List<T> list= new ArrayList<>();
	int entries=object.size();
	boolean removed=true;
	Set<T> set = new HashSet<>();
	List<T> temp= new ArrayList<>();
	if(num!=0) {
		if(num>= entries) {
			int extra= num-entries;
			while(entries>0) {
				T winnerT= object.selectWinner(removed);
				if(set.add(winnerT)) {
					list.add(winnerT);
					entries--;
				}else {
					object.addEntry(winnerT);
