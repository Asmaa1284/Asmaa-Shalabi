...
public class HomeworkM11Streams {
	...
	public static long q1() {
		// Q1: How many customers in CA?
		return customerList.stream().filter(c-> c.getState().equalsIgnoreCase("ca")).count(); // placeholder: replace with your own code
	}
	public static List<Customer> q2 () {
		// Q2: Create a list of all priority customers in MA.
		
		return customerList.stream().filter(c-> c.isPriority()== true&& c.getState().equalsIgnoreCase("MA")).collect(Collectors.toList()); // placeholder: replace with your own code	new ArrayList<Customer1>();
	}
	public static double q3() {
		// Q3: How much money have all customers spent (combined)?
		return customerList.stream().mapToDouble(Customer1::getAmountSpent).sum(); // placeholder: replace with your own code
	}
	public static double q4() {
		// Q4: How much money have all priority customers spent (combined)?
		return customerList.stream().filter(c->c.isPriority()==true).mapToDouble(Customer1::getAmountSpent).sum(); // placeholder: replace with your own code
	}
	public static Map<String, Customer1> q5() {
		// Q5: Create a map of all WY priority customers (key=id, value=customer)
		return customerList.stream().filter(c-> c.isPriority()== true&& c.getState().
				equalsIgnoreCase("WY")).
				collect(Collectors.toMap(Customer1::getId,
				c->c)); // placeholder: replace with your own code
	}
	public static double q6() {
		// Q6: What is the greatest amount of money spent by a NY priority customer?
		return customerList.stream().filter(c->c.getState().equalsIgnoreCase("NY")&&c.isPriority()==true).mapToDouble(Customer1::getAmountSpent).max().getAsDouble(); // placeholder: replace with your own code
	}
	public static String q7() {
		//Q7: Find all customers that spent > 9000.
		// Create a comma-separated String of all customer IDs for customers that spent > 9000:
		return customerList.stream().filter(c-> c.getAmountSpent()>9000).map(Customer1::getId).collect(Collectors.joining(",")); // placeholder: replace with your own code
	}
	public static void q8() {
		...
		stream.filter(c-> c.getAmountSpent()>9800).forEach(System.out::println);
	}
	public static int q9(String[] numWords) {
		// Q9: Find the sum of the numbers represented in an String array.
		return Arrays.stream(numWords).mapToInt(Integer::valueOf).sum(); // placeholder: replace with your own code
	}
	public static String q10(Integer[] nums) {
		// Q10: Create a String of the numbers represented in the array, separated by semicolons.
		return Arrays.stream(nums).map(n-> n.toString()).collect(Collectors.joining(";")); // placeholder: replace with your own code
	}
	public static void q11() {
	    ...
	    Stream.generate(()->random.nextInt(range)).filter(n-> n%3==0).limit(printnum).forEach(System.out::println);
	}
	...
	public static void q12(List<String> scrabbleWords) {
		...
		// Note: a method is provided below to convert from char to score.
		// Hint: you might consider writing another method to find the score of a word!
		scrabbleWords.stream().sorted(
				(w1,w2)-> Integer.compare(wordToScore(w2), wordToScore(w1)))
		.limit(9).forEach(w-> System.out.println(w+" worth "+wordToScore(w)+" points"));
